...
const DashboardPage = ({ user, bookings, setBookings, services, onLogout }) => {
        ...
        {activeTab === 'bookings' && (
          <div className="grid gap-6 md:grid-cols-2">
            {userBookings.length === 0 ? (
              ...
              userBookings.map(b => (
                <BookingCard key={b.id} booking={b} setBookings={setBookings} />
              ))
            )}
          </div>
        )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {services.map(s => (
              <div key={s.id} className="bg-black/70 backdrop-blur-md rounded-3xl shadow-2xl p-6 hover:scale-105 transform transition-all duration-300 border border-gray-600">
                <ServiceCard service={s} onBook={() => setShowBookingForm(true)} />
              </div>
            ))}
          </div>
        )}
